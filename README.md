...
- `FORWARD_FROM` и `FORWARD_TO` — (опционально) если заданы, скрипт будет пересылать новые сообщения из одного чата в другой.
...
- `--weekly` — включить еженедельный отчёт (по понедельникам);
- `--monthly` — включить ежемесячный отчёт (1-го числа каждого месяца);
- `--run-now` — выполнить отчёт сразу при запуске;
- `--auth` — однократный запуск для аутентификации и сохранения сессии.

## Локальный запуск

### Установка зависимостей

Не забывайте использовать виртуальное окружение Python.
